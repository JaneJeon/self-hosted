...
test('withRetry: retries a transient 500 then succeeds', async () => {
  ...
  const result = await withRetry(async () => {
    calls++
    if (calls < 3) throw httpError('Tracker API returned HTTP 500', 500)
    return 'recovered'
  }, opts({ logger }))
...
test('withRetry: exhausts attempts and rethrows the last error', async () => {
  ...
  await assert.rejects(
    withRetry(
      async () => {
        calls++
        throw httpError('Tracker API returned HTTP 500', 500)
      },
      opts({ attempts: 4 })
    ),
    /HTTP 500/
...
test('withRetry: does not retry a 4xx', async () => {
  ...
  await assert.rejects(
    withRetry(async () => {
      calls++
      throw httpError('Tracker API returned HTTP 400', 400)
    }, opts()),
    /HTTP 400/
  )
  assert.equal(calls, 1)
...
test('integration: retries a real HTTP 500 over the wire, then succeeds', async () => {
  ...
  const server = createServer((req, res) => {
    ...
    if (hits <= 2) {
      ...
      res.end('{"title":"Reset data is temporarily unavailable","status":503}')
      return
    }
    res.writeHead(200, { 'Content-Type': 'application/json' })
    res.end('{"data":{},"meta":{"api_version":"v1"}}')
  })
  ...
  try {
    ...
    assert.deepEqual(data, { data: {}, meta: { api_version: 'v1' } })
...
test('withRetry: default policy still refuses to retry a 4xx', async () => {
  ...
  await assert.rejects(
    withRetry(async () => {
      calls++
      throw httpError('Tracker API returned HTTP 404', 404)
    }, opts()),
    /HTTP 404/
  )
  assert.equal(calls, 1)
