...
# Dump all databases (--skip-ssl: Railway internal network, no TLS needed)
mysqldump --skip-ssl -h "${MYSQL_HOST}" -u root -p"${MYSQL_ROOT_PASSWORD}" --opt --all-databases > /tmp/dump.sql
